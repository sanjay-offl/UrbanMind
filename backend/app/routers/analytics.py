...
def summary(db: Session = Depends(get_db)):
    total = db.scalar(select(func.count(Grievance.id))) or 0
    open_count = db.scalar(
        select(func.count(Grievance.id)).where(Grievance.status.notin_(["resolved", "closed"]))
    ) or 0
    critical = db.scalar(
        select(func.count(Grievance.id)).where(Grievance.priority == "critical")
    ) or 0
    avg_score = db.scalar(select(func.avg(Grievance.score))) or 0.0
    categories = [
    ...
    return AnalyticsSummary(
        ...
        wards=wards,
        trends=trend_points(db, 30),
    )


def trend_points(db: Session, days: int) -> list[TrendPoint]:
    since = datetime.utcnow() - timedelta(days=days)
    rows = db.execute(
        select(func.date(Grievance.created_at).label("day"), func.count(Grievance.id))
        .where(Grievance.created_at >= since)
